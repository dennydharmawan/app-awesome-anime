...
function parseAnimePage($) {
  return {
    ...
    type: $("span:contains('Type')").get(0).nextSibling.nextSibling.firstChild
      .data,
  };
}
...
function parseVideo($, parseProvider = true) {
  const vidStreaming = $('[data-video*="//gogo-stream.com/"]').attr(
    'data-video'
  );
  ...
  console.log(`Found Vidstreaming link: ${vidStreaming}`);

  return VideoProviders.Vidstreaming('https:' + vidStreaming);
}

function parseMultiStreamVideo($) {
  const vidStreaming = $('[data-video*="//gogo"]').attr('data-video');

  if (!vidStreaming) {
    return null;
  }
  console.log(`Found Vidstreaming link: https:${vidStreaming}`);

  return { url: `https:${vidStreaming}` };
}
